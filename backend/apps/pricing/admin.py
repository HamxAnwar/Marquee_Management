...
class PricingRuleAdmin(admin.ModelAdmin):
    ...
    def get_rule_value(self, obj):
        """Display the rule value (percentage or fixed amount)"""
        if obj.percentage:
    ...
    get_rule_value.short_description = "Rule Value"
...
class PriceCalculationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Discounts", {"fields": ("guest_discount_tier", "guest_discount_amount")}),
        (
            "Additional Charges",
            {
                "fields": (
...
class BudgetSuggestionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Request Parameters", {"fields": ("target_budget", "guest_count", "hall")}),
        (
            "Suggestion Results",
            {
                "fields": (
                    "suggested_per_person_budget",
                    "total_estimated_cost",
                    "variance_percentage",
                )
            },
        ),
        ("Status", {"fields": ("is_accepted", "created_at")}),
    )
